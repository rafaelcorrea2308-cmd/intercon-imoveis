  ...
  return `
    <a class="card" href="imovel.html?id=${p.id}">
      <div class="photo-wrap">
        ...
        <span class="badge-pill">Aluguel</span>
...
function populateBairros(all) {
  const sel = document.getElementById('filter-bairro');
  const bairros = [...new Set(all.map(p => p.bairro))].sort();
...
function applyFilters(all) {
  ...
  const precoMax = document.getElementById('filter-preco').value;
  ...
  if (quartos) filtered = filtered.filter(p => p.quartos >= parseInt(quartos, 10));
  if (precoMax) filtered = filtered.filter(p => p.preco <= parseInt(precoMax, 10));
  renderGrid(filtered);
}
...
async function initIndexPage() {
  try {
    ...
    ['search', 'filter-bairro', 'filter-quartos', 'filter-preco'].forEach(id => {
      document.getElementById(id).addEventListener('input', () => applyFilters(properties));
